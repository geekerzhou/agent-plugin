...
import { getGeminiRuntimeSummary } from "./gemini-cli.mjs";
...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting gemini") || line.startsWith("turn started")) {
      return "starting";
    }
...
export function buildStatusSnapshot(cwd, options = {}) {
  ...
  return {
    workspaceRoot,
    config,
    sessionRuntime: getGeminiRuntimeSummary(options.env),
    running,
    latestFinished,
    recent,
