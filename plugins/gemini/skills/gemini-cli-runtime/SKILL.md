...
Primary helper:
- `node "${CLAUDE_PLUGIN_ROOT}/scripts/gemini-companion.mjs" run "<raw arguments>"`
...
- You may use the `agent-prompting` skill to rewrite the user's request into a tighter prompt before the single `run` call.
...
Command selection:
- Use exactly one `run` invocation per rescue handoff.
...
- `--output-format`: accepted values are `text` (default) and `json`.
