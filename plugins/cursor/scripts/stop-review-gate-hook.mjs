...
import { getAgentAuthStatus } from "./lib/cursor-agent.mjs";
...
function buildSetupNote(cwd) {
  ...
  return `Cursor agent is not set up for the review gate.${detail} Run /cursor:setup.`;
}
...
function runStopReview(cwd, input = {}) {
  ...
  const result = spawnSync(process.execPath, [scriptPath, "run", "--json", prompt], {
    cwd,
    env: childEnv,
    encoding: "utf8",
