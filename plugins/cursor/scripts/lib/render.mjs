...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.childPid) {
    lines.push(`  Agent child PID: ${job.childPid}`);
  }
...
export function renderSetupReport(report) {
  const lines = [
    ...
    `- agent binary (${report.agent.binary}): ${report.agent.detail}`,
    `- auth: ${report.auth.detail}`,
    `- runtime: ${report.cursorRuntime.label} — ${report.cursorRuntime.detail}`,
...
export function renderReviewResult(parsedResult, meta) {
  if (!parsedResult.parsed) {
    const lines = [
      ...
      "",
      "Cursor agent did not return valid structured JSON.",
      "",
  ...
  if (validationError) {
    const lines = [
      ...
      "Cursor agent returned JSON with an unexpected review shape.",
      "",
...
export function renderTaskResult(payload, meta) {
  ...
  const message = String(payload?.failureMessage ?? "").trim() || "The agent did not return stdout.";
  return `${message}\n`;
}
...
export function renderStatusReport(report) {
  const lines = [
    "# Cursor Agent Status",
    "",
    `Auth mode: ${report.cursorRuntime?.label ?? "unknown"}`,
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh Cursor review and block if it finds issues.");
  }
