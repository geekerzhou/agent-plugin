...
description: Internal guidance for composing Cursor agent prompts for coding, review, diagnosis, and research tasks inside the Cursor Claude Code plugin
...
Core rules:
- Prefer one clear task per agent run. Split unrelated asks into separate runs.
- Tell the agent what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what the agent should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so the agent stays narrow and avoids unrelated refactors.
...
- Use `run` when the task is diagnosis, planning, research, or implementation and you need to control the prompt more directly.
...
- Ask the agent for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether the agent should keep going by default or stop for missing high-risk details.
