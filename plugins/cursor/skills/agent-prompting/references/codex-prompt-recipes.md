...
Use these as starting templates for agent task prompts.
Copy the smallest recipe that fits the task, then trim anything you do not need.
In `cursor:cursor-rescue`, run diagnosis and fix-oriented recipes in write mode by default unless the user explicitly asked for read-only behavior.
