...
- For `cursor:rescue`, do not turn a failed or incomplete Cursor run into a Claude-side implementation attempt. Report the failure and stop.
- For `cursor:rescue`, if Cursor was never successfully invoked, do not generate a substitute answer at all.
